...
export const makeAttendanceRepo = (db: DB) => ({
  upsert: async (a: Omit<Attendance, "id">): Promise<Attendance> => {
    const sql = `
    ...
    const [rows] = await db.query(
      `SELECT id, student_id AS studentId, period_id AS periodId, class_id AS classId,
       ...
       FROM attendance
       WHERE student_id=? AND period_id=? AND date=?`,
      [a.studentId, a.periodId, a.date]
    );
    // @ts-ignore
    return (rows as Attendance[])[0];
  },

  list: async (filters: AttendanceFilters): Promise<Attendance[]> => {
    const where: string[] = [];
    const params: any[] = [];
    if (filters.classId) {
      where.push("class_id=?");
